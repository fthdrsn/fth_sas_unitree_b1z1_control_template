...
class ControllerNodeReconstruction(Node):
    ...
    def __init__(self):
        ...
        X_J1_OFFSET_ori=1 -3.06272860e-20*i_ -3.15365499e-21*j_ -1.11022302e-16*k_
    ...
    def execute_move_to_goal_callback(self, goal_handle):
        ...
        try:
            ...
            print("Robot model updated with current state for goal execution.")
            ...
            while rclpy.ok() and task_error > error_tolerance:
                try:
                    ...
                    if method_to_use == 0:
                        ...
                        self.get_logger().error(f'Invalid method_to_use value: {method_to_use}. Expected 0, 1, or 2.')
                        self.stop_robot_motion(repetitions=10, timeout_sec=0.01)
                        goal_handle.abort()
                        result = MoveToGoal.Result()
                        result.success = False
